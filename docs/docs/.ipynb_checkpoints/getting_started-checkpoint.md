...
curl -X POST http://localhost:8080/a2ui/action \
  -H "Content-Type: application/json" \
  -d '{"surfaceId": "surf_abc", "componentId": "btn-save", "action": "click"}'
```
...
```bash
# Create an intent mandate (owner sets budget + allowed merchants)
curl -X POST http://localhost:8080/ap2/mandates/intent \
  -H "Content-Type: application/json" \
  -d '{
    "ownerId": "user-123",
...
# Create a payment mandate (agent requests payment)
curl -X POST http://localhost:8080/ap2/mandates/payment \
  -H "Content-Type: application/json" \
  -d '{
    "intentMandateId": "{intentId}",
    "merchantId": "merchant-A",
    "totalAmount": {"amount": "20.00", "currency": "USD"},
    "items": [{"name": "Service", "amount": {"amount": "20.00", "currency": "USD"}}],
    "status": "pending_approval"
  }'

# View audit trail
curl http://localhost:8080/ap2/audit/{intentId}
```

Payments below `autoApproveBelow` are auto-approved. Others require explicit approval via `POST /ap2/mandates/payment/{id}/approve`.
...
```
Input -> PERCEIVE -> ADAPT -> REASON -> PLAN -> EXECUTE -> LEARN -> Response
```
...
3. **Reason** -- Select and apply a reasoning strategy (Chain-of-Thought, ReAct, Reflexion, Internal Debate)
4. **Plan** -- Generate an execution plan with tool calls, dependencies, and parallelization
5. **Execute** -- Run the plan via MCP tools, with retry and fallback logic
6. **Learn** -- Store the episode in memory, update the caller profile, self-evaluate
